...
func (v *PlaidErrorType) UnmarshalJSON(src []byte) error {
	...
	return fmt.Errorf("%+v is not a valid PlaidErrorType", value)
}
...
func NewPlaidErrorTypeFromValue(v string) (*PlaidErrorType, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for PlaidErrorType: valid values are %v", v, allowedPlaidErrorTypeEnumValues)
	}
}
