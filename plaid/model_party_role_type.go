...
func (v *PartyRoleType) UnmarshalJSON(src []byte) error {
	...
	return fmt.Errorf("%+v is not a valid PartyRoleType", value)
}
...
func NewPartyRoleTypeFromValue(v string) (*PartyRoleType, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for PartyRoleType: valid values are %v", v, allowedPartyRoleTypeEnumValues)
	}
}
