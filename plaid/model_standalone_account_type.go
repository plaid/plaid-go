...
type StandaloneAccountType struct {
	// An account type holding cash, in which funds are deposited. Supported products for `depository` accounts are: Auth (`checking` and `savings` subtypes only), Transfer, Balance, Signal, Income, Transactions, Identity, Payment Initiation, Assets, and Investments (`cash management` subtype only).
	...
	// A loan type account. Supported products for `loan` accounts are: Balance, Liabilities (`student` and `mortgage` subtypes only), Identity.
	Loan string `json:"loan"`
	// An investment account. Supported products for `investment` accounts are: Balance, Assets, Investments, and Investment Transactions. In API versions 2018-05-22 and earlier, this type is called `brokerage`.
	Investment string `json:"investment"`
	// Other or unknown account type. Supported products for `other` accounts are: Balance, Identity.
