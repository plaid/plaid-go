...
type InstitutionsSearchRequest struct {
	...
	// Filter the Institutions based on whether they support all products listed in `products`. Provide `null` to get institutions regardless of supported products. Note that when `auth` is specified as a product, if you are enabled for Instant Match or Automated Micro-deposits, institutions that support those products will be returned even if `auth` is not present in their product array. To search for Transfer support, use `auth`; to search for Signal support, use `balance`.
...
func NewInstitutionsSearchRequest(query string, countryCodes []CountryCode) *InstitutionsSearchRequest {
...
func (o *InstitutionsSearchRequest) HasProducts() bool {
	if o != nil && o.Products != nil {
		return true
	}
