/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * API version: 2020-09-14_1.680.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package plaid

import (
	"encoding/json"
)

// ItemRemoveRequest ItemRemoveRequest defines the request schema for `/item/remove`
type ItemRemoveRequest struct {
	// Your Plaid API `client_id`. The `client_id` is required and may be provided either in the `PLAID-CLIENT-ID` header or as part of a request body.
	ClientId *string `json:"client_id,omitempty"`
	// Your Plaid API `secret`. The `secret` is required and may be provided either in the `PLAID-SECRET` header or as part of a request body.
	Secret *string `json:"secret,omitempty"`
	// The access token associated with the Item data is being requested for.
	AccessToken string `json:"access_token"`
	ReasonCode NullableItemRemoveReasonCode `json:"reason_code,omitempty"`
	// Additional context or details about the reason for removing the item. Personally identifiable information, such as an email address or phone number, should not be included in the `reason_note`.
	ReasonNote NullableString `json:"reason_note,omitempty"`
}

// NewItemRemoveRequest instantiates a new ItemRemoveRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewItemRemoveRequest(accessToken string) *ItemRemoveRequest {
	this := ItemRemoveRequest{}
	this.AccessToken = accessToken
	return &this
}

// NewItemRemoveRequestWithDefaults instantiates a new ItemRemoveRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewItemRemoveRequestWithDefaults() *ItemRemoveRequest {
	this := ItemRemoveRequest{}
	return &this
}

// GetClientId returns the ClientId field value if set, zero value otherwise.
func (o *ItemRemoveRequest) GetClientId() string {
	if o == nil || o.ClientId == nil {
		var ret string
		return ret
	}
	return *o.ClientId
}

// GetClientIdOk returns a tuple with the ClientId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ItemRemoveRequest) GetClientIdOk() (*string, bool) {
	if o == nil || o.ClientId == nil {
		return nil, false
	}
	return o.ClientId, true
}

// HasClientId returns a boolean if a field has been set.
func (o *ItemRemoveRequest) HasClientId() bool {
	if o != nil && o.ClientId != nil {
		return true
	}

	return false
}

// SetClientId gets a reference to the given string and assigns it to the ClientId field.
func (o *ItemRemoveRequest) SetClientId(v string) {
	o.ClientId = &v
}

// GetSecret returns the Secret field value if set, zero value otherwise.
func (o *ItemRemoveRequest) GetSecret() string {
	if o == nil || o.Secret == nil {
		var ret string
		return ret
	}
	return *o.Secret
}

// GetSecretOk returns a tuple with the Secret field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ItemRemoveRequest) GetSecretOk() (*string, bool) {
	if o == nil || o.Secret == nil {
		return nil, false
	}
	return o.Secret, true
}

// HasSecret returns a boolean if a field has been set.
func (o *ItemRemoveRequest) HasSecret() bool {
	if o != nil && o.Secret != nil {
		return true
	}

	return false
}

// SetSecret gets a reference to the given string and assigns it to the Secret field.
func (o *ItemRemoveRequest) SetSecret(v string) {
	o.Secret = &v
}

// GetAccessToken returns the AccessToken field value
func (o *ItemRemoveRequest) GetAccessToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AccessToken
}

// GetAccessTokenOk returns a tuple with the AccessToken field value
// and a boolean to check if the value has been set.
func (o *ItemRemoveRequest) GetAccessTokenOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.AccessToken, true
}

// SetAccessToken sets field value
func (o *ItemRemoveRequest) SetAccessToken(v string) {
	o.AccessToken = v
}

// GetReasonCode returns the ReasonCode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ItemRemoveRequest) GetReasonCode() ItemRemoveReasonCode {
	if o == nil || o.ReasonCode.Get() == nil {
		var ret ItemRemoveReasonCode
		return ret
	}
	return *o.ReasonCode.Get()
}

// GetReasonCodeOk returns a tuple with the ReasonCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ItemRemoveRequest) GetReasonCodeOk() (*ItemRemoveReasonCode, bool) {
	if o == nil  {
		return nil, false
	}
	return o.ReasonCode.Get(), o.ReasonCode.IsSet()
}

// HasReasonCode returns a boolean if a field has been set.
func (o *ItemRemoveRequest) HasReasonCode() bool {
	if o != nil && o.ReasonCode.IsSet() {
		return true
	}

	return false
}

// SetReasonCode gets a reference to the given NullableItemRemoveReasonCode and assigns it to the ReasonCode field.
func (o *ItemRemoveRequest) SetReasonCode(v ItemRemoveReasonCode) {
	o.ReasonCode.Set(&v)
}
// SetReasonCodeNil sets the value for ReasonCode to be an explicit nil
func (o *ItemRemoveRequest) SetReasonCodeNil() {
	o.ReasonCode.Set(nil)
}

// UnsetReasonCode ensures that no value is present for ReasonCode, not even an explicit nil
func (o *ItemRemoveRequest) UnsetReasonCode() {
	o.ReasonCode.Unset()
}

// GetReasonNote returns the ReasonNote field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ItemRemoveRequest) GetReasonNote() string {
	if o == nil || o.ReasonNote.Get() == nil {
		var ret string
		return ret
	}
	return *o.ReasonNote.Get()
}

// GetReasonNoteOk returns a tuple with the ReasonNote field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ItemRemoveRequest) GetReasonNoteOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.ReasonNote.Get(), o.ReasonNote.IsSet()
}

// HasReasonNote returns a boolean if a field has been set.
func (o *ItemRemoveRequest) HasReasonNote() bool {
	if o != nil && o.ReasonNote.IsSet() {
		return true
	}

	return false
}

// SetReasonNote gets a reference to the given NullableString and assigns it to the ReasonNote field.
func (o *ItemRemoveRequest) SetReasonNote(v string) {
	o.ReasonNote.Set(&v)
}
// SetReasonNoteNil sets the value for ReasonNote to be an explicit nil
func (o *ItemRemoveRequest) SetReasonNoteNil() {
	o.ReasonNote.Set(nil)
}

// UnsetReasonNote ensures that no value is present for ReasonNote, not even an explicit nil
func (o *ItemRemoveRequest) UnsetReasonNote() {
	o.ReasonNote.Unset()
}

func (o ItemRemoveRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.ClientId != nil {
		toSerialize["client_id"] = o.ClientId
	}
	if o.Secret != nil {
		toSerialize["secret"] = o.Secret
	}
	if true {
		toSerialize["access_token"] = o.AccessToken
	}
	if o.ReasonCode.IsSet() {
		toSerialize["reason_code"] = o.ReasonCode.Get()
	}
	if o.ReasonNote.IsSet() {
		toSerialize["reason_note"] = o.ReasonNote.Get()
	}
	return json.Marshal(toSerialize)
}

type NullableItemRemoveRequest struct {
	value *ItemRemoveRequest
	isSet bool
}

func (v NullableItemRemoveRequest) Get() *ItemRemoveRequest {
	return v.value
}

func (v *NullableItemRemoveRequest) Set(val *ItemRemoveRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableItemRemoveRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableItemRemoveRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableItemRemoveRequest(val *ItemRemoveRequest) *NullableItemRemoveRequest {
	return &NullableItemRemoveRequest{value: val, isSet: true}
}

func (v NullableItemRemoveRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableItemRemoveRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


