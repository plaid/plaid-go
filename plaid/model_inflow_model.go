...
// InflowModel The `inflow_model` allows you to model a test account that receives regular income or makes regular payments on a loan. Any transactions generated by the `inflow_model` will appear in addition to randomly generated test data or transactions specified by `override_accounts`.
type InflowModel struct {
	...
	// Number between 1 and 28, or `last` meaning the last day of the month. The day of the month on which the income transaction will appear. This field is required if `type` is `monthly-income`, `monthly-balance-payment` or `monthly-interest-only-payment`.
	...
	// Number between 1 and 28, or `last` meaning the last day of the month. The day of the month on which the balance is calculated for the next payment. This field is required if `type` is `monthly-balance-payment` or `monthly-interest-only-payment`.
...
func NewInflowModel(type_ string, incomeAmount float64, paymentDayOfMonth float32, transactionName string, statementDayOfMonth string) *InflowModel {
	this := InflowModel{}
	this.Type = type_
...
func (o *InflowModel) GetIncomeAmount() float64 {
	if o == nil {
		var ret float64
		return ret
	}
...
// and a boolean to check if the value has been set.
func (o *InflowModel) GetIncomeAmountOk() (*float64, bool) {
	if o == nil  {
		return nil, false
	}
...
func (o *InflowModel) SetIncomeAmount(v float64) {
	o.IncomeAmount = v
}
