...
type InstitutionsGetRequestOptions struct {
	// Filter the Institutions based on which products they support. Will only return institutions that support all listed products. When filtering based on `auth`, an institution must support Instant Auth to match the criterion. To filter for Signal support, use `balance`. To filter for Transfer support, use `auth`.
