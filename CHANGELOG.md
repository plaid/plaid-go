...
- [BREAKING] This is a new Go major version. Update `go.mod` and imports from `github.com/plaid/plaid-go/v44` to `github.com/plaid/plaid-go/v45`.
- [BREAKING] `/cra/report/get` product `Metadata` and `Attributes`, plus `/cra/credit_profile/report/get` cash flow and network insight attribute maps, now use `map[string]interface{}` instead of `map[string]map[string]interface{}` so valid scalar, array, object, and null values deserialize correctly.
...
## Breaking changes in this version
- [BREAKING] This is a new Go major version. Update `go.mod` and imports from `github.com/plaid/plaid-go/v43` to `github.com/plaid/plaid-go/v44`.
...
- [BREAKING] Use `ProductsTerminateReasonCode` and the `PRODUCTSTERMINATEREASONCODE_*` constants for both Item and user product termination; the endpoint-specific wrapper and nullable types were removed. Accepted wire values are unchanged. (OAS 2020-09-14_1.701.1)
- [BREAKING] Calls to `NewBankTransferEvent`, `NewCreditSessionDocumentIncomeResult`, `NewLinkEventsWebhook`, `NewPayrollIncomeObject`, and `NewSecurity`, as well as unkeyed literals for those models, must supply their newly required field. Keyed struct literals remain compatible. (OAS 2020-09-14_1.699.0, 1.699.5, 1.701.0, 1.701.1, and 1.703.0)
- [BREAKING] `FDXInitiatorFiAttribute.Value`, its getter, and its setter now use `*string` instead of `*FDXPartyType`. This schema is not referenced by an endpoint. (OAS 2020-09-14_1.703.0)
...
## Breaking changes in this version
- [BREAKING] This is a new Go major version (`github.com/plaid/plaid-go/v43`). Update your import paths from `.../plaid-go/v42` to `.../plaid-go/v43`.
...
- [BREAKING for Go] For CRA endpoints (`/cra/check_report/income_insights/get`, `/cra/check_report/base_report/get`, `/cra/check_report/partner_insights/get`) made `user_token` an optional field to support third party user tokens (beta) as an alternative.
...
- Mark `products`, `required_if_supported_products`, `optional_products`, `additional_consented_products`, and `user.ssn` as nullable in `/link/token/create` request.
- Mark `access_token` and `account_id` required in `/transfer/create`, to match the actual behavior of the API.
...
- Update `/transfer/capabilities/get` to no longer accept payment profile token and require an `account_id` and `access_token`
- Remove `device`, `user_present` from required request param list of `/transfer/recurring/create`
...
- For Transfer endpoints that take payment profiles as input, remove `payment_profile` field and make
  `account_id` and `access_token` mandatory.
...
- Updating to OAS 2020-09-14_1.391.2

## Breaking changes in this version

- Make `asset_report_token` an optional field in `/asset_report/get` and mark as nullable
...
- Make `virtual_time` optional in `/sandbox/transfer/test_clock/create`
...
- Type changes for some returned fields that were incorrectly typed as floats when they should have been integers. These fields were changed from `Float64` to `Int32`. For details, see the change message for 2020-09-14_1.349.0.
...
- Made access_tokens an optional parameter for /asset_report/create to support user-token-based asset reports
...
- (Payment Initiation) Make `payment_id` not required in `/link/token/create` under the `payment_initiation` field.
...
- Rename `LOGIN_REQUIRED` from transfer authorizations `decision_rationale` to `ITEM_LOGIN_REQUIRED`
- (Identity Verification) Update `IdentityVerificationRequestUser.date_of_birth` to be required to match the existing behavior of the API.
- (Identity Verification and Monitor) Remove `Date` and `DateNullable` types in identity_verification and monitor endpoints. Replace with `ISO8601Date` and `ISO8601DateNullable`
  instead.
...
- bump golang.org/x/oauth version
- bump github.com/stretchr/testify version
- bump (indirect dependency) golang.org/x/net version
- bump (indirect dependency) golang.org/x/crypto => golang.org/x/crypto version
...
- Note: we're updating the version from v3.5.0 to v8.0.0 because of this issue [here](https://github.com/plaid/plaid-go/issues/235). This should help make this package's versioning a lot more friendly for dependabot and other tools that use tags for version comparison. Going forward, we should be following the typical versioning system and not skip any other major versions.
- Updating to OAS 2020-09-14_1.131.3

## Breaking changes in this version

This version does not contain breaking changes. The version bump was required for version management reasons described above.
...
This version represents a transition in how we maintain our external client libraries. We are now using an [API spec](https://github.com/plaid/plaid-openapi) written in `OpenAPI 3.0.0` and running our definition file through [OpenAPITool's `go` generator](https://github.com/OpenAPITools/openapi-generator). See the [migration guide](https://github.com/plaid/plaid-go#migration-guide) for instructions on migrating to this version.
