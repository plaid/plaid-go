...
func TestPayStubsGet(t *testing.T) {
	...
	resp, localReturn, err := testClient.PlaidApi.IncomeVerificationPaystubsGet(ctx).IncomeVerificationPaystubsGetRequest(*request).Execute()
	fmt.Printf("lebron %+v\n", resp)
	fmt.Printf("lebron2 %+v\n", localReturn)
	fmt.Printf("lebron3 %+v\n", err)
